...
export const noteFormSchema = z.object({
  title: z
    .string()
    .min(1, '제목을 입력해주세요')
    .max(NOTE_TITLE_MAX_LENGTH, `제목은 최대 ${NOTE_TITLE_MAX_LENGTH}자까지 입력 가능합니다`),
  content: z
    .string()
    .max(NOTE_CONTENT_MAX_LENGTH, `본문은 최대 ${NOTE_CONTENT_MAX_LENGTH}자까지 입력 가능합니다`)
    .default(''),
...
export function useNoteForm(param?: UseNoteFormParam) {
  ...
  return useForm<NoteFormData>({
    ...
    defaultValues: {
      ...
      categoryId: existingNote?.categoryId ?? defaultValue?.categoryId ?? undefined,
    },
  });
}
