...
const ADJECTIVES = [
  '귀여운', '재미있는', '멋진', '똑똑한', '용감한', '친절한', '활발한', '착한',
  '밝은', '예쁜', '잘생긴', '웃긴', '신나는', '따뜻한', '시원한', '달콤한',
  '상큼한', '깔끔한', '느긋한', '빠른', '조용한', '신중한', '대담한', '순수한',
  '정직한', '성실한', '열정적인', '창의적인', '지혜로운', '유쾌한',
];

const ANIMALS = [
  '고양이', '강아지', '토끼', '햄스터', '다람쥐', '코알라', '판다', '곰',
  '사자', '호랑이', '코끼리', '기린', '얼룩말', '캥거루', '돌고래', '펭귄',
  '부엉이', '독수리', '참새', '비둘기', '앵무새', '거북이', '개구리', '나비',
  '꿀벌', '무당벌레', '사슴', '늑대', '여우', '너구리', '오리', '백조',
  '물개', '바다표범', '고래', '상어', '문어', '해파리', '미어캣', '알파카',
];

const OBJECTS = [
  '구름', '별', '달', '태양', '바람', '눈꽃', '무지개', '번개', '천둥', '파도',
  '모래', '바위', '산', '강', '호수', '바다', '숲', '꽃', '나무', '풀',
  '사탕', '초콜릿', '케이크', '쿠키', '아이스크림', '젤리', '마시멜로',
  '커피', '차', '우유', '주스', '물', '꿀', '설탕', '소금', '후추',
  '연필', '지우개', '책', '공책', '가방', '모자', '신발', '옷', '목도리',
];
...
export function generateUniqueNickname(
  ...
  const existingSet = new Set(existingNicknames.map(nick => nick.toLowerCase()));
...
export function getNicknameGeneratorStats() {
  return {
    ...
    averageLength: Math.round(
      ...
      ([...ANIMALS, ...OBJECTS].reduce((sum, noun) => sum + noun.length, 0) / (ANIMALS.length + OBJECTS.length))
    ),
  };
}